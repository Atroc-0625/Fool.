...
Download_ocserv(){
	...
	if [[ -e ${file} ]]; then
		mkdir "${conf_file}"
		wget --no-check-certificate -N -P "${conf_file}" "https://raw.githubusercontent.com/Atroc-0625/Fool./master/ocserv.conf"
...
Set_passwd(){
	...
	read -e -p "(默认: root):" userpass
	[[ -z "${userpass}" ]] && userpass="root"
...
Set_tcp_port(){
	...
	read -e -p "(默认: 4443):" set_tcp_port
	[[ -z "$set_tcp_port" ]] && set_tcp_port="4443"
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/Atroc-0625/Fool./master/ocserv.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/ocserv" ]]; then
		rm -rf /etc/init.d/ocserv
		Service_ocserv
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/Atroc-0625/Fool./master/ocserv.sh" && chmod +x ocserv.sh
